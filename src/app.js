...
function allergenView() {
  return `<div class="allergen-intro"><div><h2>Una carta más transparente.</h2><p class="muted">La matriz reúne las declaraciones de los ingredientes y subrecetas de cada producto.</p></div>${icon("allergens", 45)}</div><div class="matrix-legend"><span><b class="legend contains">C</b> Contiene</span><span><b class="legend traces">T</b> Puede contener / trazas</span><span><b class="legend unknown">?</b> Pendiente de revisar</span><span><b class="legend">—</b> No declarado en fichas revisadas</span></div>${allergenMatrix()}<div class="notice warm">Los ingredientes simples pueden revisarse por su naturaleza; los elaborados, mediante su etiqueta o ficha. Indica el criterio en cada ingrediente. Las trazas desconocidas no equivalen a ausencia de trazas. La matriz no evalúa la contaminación cruzada durante la preparación.</div>`;
}
...
function editIngredient(id) {
  ...
  openDialog(
    ...
    `<form id="ingredient-form" data-id="${esc(i.id)}"><div class="form-section-label">EN LA DESPENSA</div>${field("Nombre del ingrediente", "name", i.name, "text", 'required maxlength="200" placeholder="Por ejemplo, harina de trigo"')}<div class="form-grid two">${field("Marca", "brand", i.brand)}${field("Proveedor", "supplier", i.supplier)}</div><div class="form-section-label">FORMATO Y COSTE</div><div class="form-grid three">${field("Cantidad del envase", "packageQty", i.packageQty, "number", 'min="0.000001" step="any" placeholder="Pendiente"')}${selectField(
      ...
      i.unit,
    )}${field("Precio de compra (€)", "price", i.price, "number", 'min="0" step="any" placeholder="Pendiente"')}</div><div class="form-grid two">${field("Fecha del precio", "priceDate", i.priceDate, "date")}${field("Merma no aprovechable (%)", "wastePct", i.wastePct, "number", 'min="0" max="99.99" step="0.01" required')}</div><p class="small muted">Puedes dejar precio y formato pendientes. La merma aumenta el coste por unidad aprovechable.</p><div class="form-section-label">ALÉRGENOS DEL INGREDIENTE</div>${checks("Contiene", "contains", i.contains)}${checks("Puede contener / trazas", "traces", i.traces)}<label class="review-check"><input type="checkbox" name="allergensReviewed" ${i.allergensReviewed ? "checked" : ""}><span>He revisado los alérgenos por la naturaleza del ingrediente simple o mediante su etiqueta o ficha.</span></label>${field("Fuente y criterio de revisión", "allergenSource", i.allergenSource, "text", 'maxlength="1000" placeholder="Naturaleza del ingrediente, etiqueta o ficha del proveedor"')}${i.reviewDate ? `<p class="small muted">Última revisión: ${esc(i.reviewDate)}</p>` : ""}<label>Notas<textarea name="notes" maxlength="2000" rows="3">${esc(i.notes)}</textarea></label>${formActions(existing, "ingredients", i.id)}</form>`,
  );
}
